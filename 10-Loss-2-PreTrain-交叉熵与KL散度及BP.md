...
此外，根据两个分布p和q的位置，KL散度还区分正向和反向。
## 3.3 正向KL散度
即原始KL散度，如下
$$D_{KL}(p||q) = \sum_{i=1}^n p(y_i)\log \frac{p(y_i)}{q(y_i)}$$
应用场景：
- 变分推断：在 VAE 等模型中，通常使用 正向 KL 散度 来最小化近似后验与先验之间的差异。
- 语言模型：在 训练生成模型（如 GPT）时，模型的目标是最小化生成文本的 交叉熵损失，从而实现正向 KL 散度的优化。
## 3.4 反向KL散度
调换pq位置，如下
$$D_{KL}(q||p) = \sum_{i=1}^n q(y_i)\log \frac{q(y_i)}{p(y_i)}$$
应用场景：
- 生成对抗网络（GAN）：在 GAN 中，生成器的目标是最小化 反向 KL 散度，以使生成分布尽可能接近真实分布 
- 强化学习：在 策略优化 中，尤其是当我们希望 最大化策略的覆盖范围 时，使用 反向 KL 散度 来优化新旧策略的关系。在一些强化学习算法（如 DPO）中，使用反向 KL 散度作为损失函数，用来调节策略之间的变化。
